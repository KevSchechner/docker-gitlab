...
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:14.4.1 app:rake gitlab:backup:create
```
## Restoring Backups
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:14.4.1 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:14.4.1 app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker pull sameersbn/gitlab:14.4.1
```
